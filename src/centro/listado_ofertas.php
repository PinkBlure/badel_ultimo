...
// Verificar si se presionó el botón de verificación
if (isset($_POST['verificar_oferta'])) {
  ...
  try {
    ...
    // Actualizar el estado de verificación de la oferta
    $query = "UPDATE oferta_empleo SET verificado = 1 WHERE id = :oferta_id";
...
<body>
  ...
  <main class="container my-5">
    <h1>Listado de ofertas no verificadas</h1>
    <div class="row">
      <?php
      ...
      try {
        ...
        $query = "SELECT * FROM oferta_empleo WHERE verificado=0 ORDER BY fecha_publicacion DESC";
        $stmt = $conn->prepare($query);
        $stmt->execute();
...
<div class="card h-100" style="border: 2px solid #5bc0de; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 123, 255, 0.1);">
  <a href="./oferta_detalle.php?slug=' . urlencode($oferta['slug']) . '" class="text-decoration-none">
    <div class="card-header" style="background-color: #f8f9fa; border-bottom: 1px solid #ddd; border-top-left-radius: 10px; border-top-right-radius: 10px;">
      ...
      <form action="../assets/function/cambiar_estado_verificado_oferta.php" method="POST">
    <input type="hidden" name="oferta_id" value="' . $oferta['id'] . '">
    <button type="submit" name="verificar_oferta" class="btn" style="background-color: #5bc0de;">Verificar</button>
  </form>
    </div>
